...
- `POST /api/onboard` — audits the URL via the Node engine, asks Claude to
  infer name/description/brand-voice/product-info, creates the company +
  documents, seeds the Actions Feed with the audit summary, redirects to the
  dashboard.
...
- `POST /api/chat` — streamed Claude reply (SSE) grounded on the live company
  context (company + pending proposals + latest audit + documents).
...
autoseo-web (Next.js, this app)
  ...
  ├── app/api/chat              streamed Claude reply over SSE
  ├── lib/supabase/server.ts    service-role client (server-only)
  ├── lib/anthropic.ts          Claude client (server-only)
...
autoseo-app (Node Express, the SEO/GEO engine)
   ...
   ├── runAudit() — auditors/* → prioritize → solver (Claude or rule-based)
...
5. Click **Run SEO + GEO audit**. New proposals appear in the feed grouped
   ...
   the *Archived* tab).
6. Chat panel: ask "What should I ship first?" → Claude responds, citing the
   current audit score.
...
- `ANTHROPIC_API_KEY` missing → onboarding uses fallback name/description;
  chat returns a 500 with a clear error.
